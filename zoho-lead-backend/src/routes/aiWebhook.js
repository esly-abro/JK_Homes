...
async function updateLeadFromConversation(leadId, conversationData) {
    try {
        const { transcript, analysis, bookingInfo, duration } = conversationData;
        ...
        if (bookingInfo.siteVisitBooked) {
            ...
            updateData.Budget_Range = bookingInfo.budget;
            updateData.Property_Type = bookingInfo.propertyType;

            logger.info('🎉 Site visit booked via AI call', {
                leadId,
                date: bookingInfo.visitDate,
                time: bookingInfo.visitTime
            });
            ...
            updateData.Preferred_Contact_Method = bookingInfo.preferredContact || 'Phone';

        } else if (bookingInfo.notInterested) {
            updateData.Lead_Status = 'Not Interested';
            updateData.Not_Interested_Reason = bookingInfo.reason || 'Not interested at this time';
        } else {
        ...
        logger.info('Lead updated with AI conversation analysis', {
            ...
            siteVisitBooked: bookingInfo.siteVisitBooked
        });
